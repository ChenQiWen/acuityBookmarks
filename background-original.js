        ...
        try {
          ...
          chrome.storage.local.get(['localDataStatus', 'lastLocalUpdate', 'localBookmarkCount'], async (data) => {
            ...
            if (bookmarks.length > 0 && data.localDataStatus === 'ready') {
              ...
              // 发送响应给popup
              sendResponse({ success: true, status: 'page_opened_with_cache' });
              return;
            }
            ...
            if (bookmarks.length > 0 && data.localDataStatus !== 'ready') {
              ...
              // 发送响应给popup
              sendResponse({ success: true, status: 'page_opened_with_recovery' });
              return;
            }
          ...
          chrome.bookmarks.getTree(tree => {
            ...
            // 发送响应给popup
            sendResponse({ success: true, status: 'page_opened_with_error_fallback' });
          });
        }
      }, 100); // 短暂延迟让页面先加载
