  ...
  async function loadCachedAnalysis(): Promise<void> {
    try {
      if (typeof chrome !== 'undefined' && chrome.storage) {
        const result = await chrome.storage.local.get(['ai_analysis_cache']);
        if (result.ai_analysis_cache) {
          aiAnalysisCache.value = new Map(result.ai_analysis_cache);
        }
      }
    } catch (error) {
    ...
    try {
      if (typeof chrome !== 'undefined' && chrome.storage) {
        await chrome.storage.local.set({
          ai_analysis_cache: Array.from(aiAnalysisCache.value.entries())
        });
      }
    } catch (error) {
