          ...
          <slot name="item" :result="result" :index="index">
            <div class="search-result-content">
              ...
              <div class="search-result-info">
                ...
                <div
                  v-if="showPath && result.path?.length"
...
const displayItemsWithKey = computed(() => {
  const list = displayItems.value.map((item, index) => ({
    item,
    index,
    key: stableKeyOf(item)
  }))
  ...
  if (!isProd) {
    for (const { item } of list) {
      if (!item.id && !item.url) {
        console.warn(
          '[SearchResultsDropdown] item missing stable id/url; a hash key is used as fallback',
          item
        )
        break
      }
