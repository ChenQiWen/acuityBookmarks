...
export class SmartBookmarkDiffEngine {
  ...
  async computeDiff(originalTree: BookmarkNode[], targetTree: BookmarkNode[]): Promise<DiffResult> {
  ...
  private async performTreeDiff(
    ...
    operations.push(...(await this.findMoveOperations(originalTree, targetTree)))

    return operations
  }

  private findDeleteOperations(originalMap: Map<string, BookmarkNode>, targetMap: Map<string, BookmarkNode>): BookmarkOperation[] {
    const operations: BookmarkOperation[] = []
  ...
  private findCreateOperations(originalMap: Map<string, BookmarkNode>, targetMap: Map<string, BookmarkNode>): BookmarkOperation[] {
    const operations: BookmarkOperation[] = []
    targetMap.forEach((node, id) => {
      if (!originalMap.has(id)) {
        operations.push({
          ...
          priority: 10,
          target: { title: node.title, url: node.url, parentId: node.parentId, index: node.index },
          estimatedCost: 15
        })
      }
    })
  ...
  private findUpdateOperations(originalMap: Map<string, BookmarkNode>, targetMap: Map<string, BookmarkNode>): BookmarkOperation[] {
    const operations: BookmarkOperation[] = []
    targetMap.forEach((targetNode, id) => {
      const originalNode = originalMap.get(id)
      if (originalNode) {
        if (originalNode.title !== targetNode.title) {
          operations.push({ id: `update_title_${this.operationCounter++}`, type: OperationType.UPDATE, priority: 20, nodeId: id, target: { id, title: targetNode.title }, estimatedCost: 8 })
        }
        if (originalNode.url !== targetNode.url && targetNode.url) {
          operations.push({ id: `update_url_${this.operationCounter++}`, type: OperationType.UPDATE, priority: 25, nodeId: id, target: { id, url: targetNode.url }, estimatedCost: 8 })
        }
      }
    })
    return operations
  ...
  private async findMoveOperations(originalTree: BookmarkNode[], targetTree: BookmarkNode[]): Promise<BookmarkOperation[]> {
    const operations: BookmarkOperation[] = []
  ...
  private async analyzeFolderReordering(
    ...
    if (moveSequence.length > 0) {
      if (moveSequence.length > 3) {
        operations.push({ id: `reorder_${parentId}_${this.operationCounter++}`, type: OperationType.REORDER, priority: 50, target: { parentId, children: targetChildren }, estimatedCost: moveSequence.length * 5 })
      } else {
        moveSequence.forEach(move => {
          operations.push({ id: `move_${move.nodeId}_${this.operationCounter++}`, type: OperationType.MOVE, priority: 40, nodeId: move.nodeId, target: { id: move.nodeId, parentId, index: move.toIndex }, estimatedCost: 12 })
        })
      }
    }

    const allFolders = new Set([...originalChildren, ...targetChildren].filter(n => n.children).map(n => n.id!))
    for (const folderId of allFolders) {
      await this.analyzeFolderReordering(folderId, originalParentMap, targetParentMap, operations)
    }
  }

  private calculateOptimalMoveSequence(original: BookmarkNode[], target: BookmarkNode[]): Array<{ nodeId: string; fromIndex: number; toIndex: number }> {
    ...
    for (let targetIndex = 0; targetIndex < target.length; targetIndex++) {
      ...
      if (originalIndex !== -1 && originalIndex !== targetIndex) {
        moves.push({ nodeId: targetNode.id!, fromIndex: originalIndex, toIndex: targetIndex })
      }
    }
    return moves
  }
  ...
  private analyzeDependencies(operations: BookmarkOperation[]): void {
    operations.forEach(op => {
      if (op.type === OperationType.MOVE && op.target?.parentId) {
        const parentCreateOp = operations.find(other => other.type === OperationType.CREATE && other.target?.id === op.target?.parentId)
        if (parentCreateOp) {
  ...
  private determineStrategy(operations: BookmarkOperation[]): DiffResult['strategy'] {
    ...
    if (totalOps < 10 && complexity !== 'extreme') {
      return { type: 'incremental', reason: '变更较少，适合增量更新', recommendations: ['使用单个Chrome API调用', '实时反馈用户进度'] }
    } else if (totalOps < 100 && complexity !== 'extreme') {
      return { type: 'batch', reason: '中等规模变更，适合批量处理', recommendations: ['使用批量API调用', '分批执行避免阻塞', '显示详细进度条'] }
    } else {
      return { type: 'rebuild', reason: '大规模变更，建议重建书签树', recommendations: ['先备份原有书签', '清空后重新构建', '提供回滚机制'] }
    }
  }
  ...
  private calculateStats(operations: BookmarkOperation[]): DiffResult['stats'] {
    ...
    const apiCalls = operations.filter(op => op.type !== OperationType.REORDER).length + operations.filter(op => op.type === OperationType.REORDER).length * 3
    return { totalOperations: operations.length, estimatedTime: totalTime, apiCalls, complexity: this.calculateComplexity(operations) }
  }
  ...
  private buildParentChildMap(tree: BookmarkNode[]): Map<string, BookmarkNode[]> {
    ...
    const traverse = (nodes: BookmarkNode[], parentId = 'root') => {
      ...
      nodes.forEach(node => { if (node.children && node.id) traverse(node.children, node.id) })
    }
    traverse(tree)
    return map
  }
