...
export class BookmarkPreprocessor {
  ...
  async processBookmarks(
    ...
    try {
      ...
      const enhancedBookmarks = await this._enhanceBookmarks(
        flatBookmarks,
        opts
      )
      ...
      throw new Error(
        `书签预处理失败: ${error instanceof Error ? error.message : String(error)}`
      )
    }
  }
  ...
  private _enhanceSingleBookmark(
    ...
    // 生成搜索关键词
    const keywords = this._generateKeywords(
      node.title,
      node.url,
      domain,
      options.maxKeywordsPerBookmark
    )
  ...
  private _categorizeDomain(domain: string): string {
    ...
    // 搜索引擎
    if (
      ['google.com', 'bing.com', 'baidu.com', 'duckduckgo.com'].includes(domain)
    ) {
      return 'search'
    }
    ...
    // 视频平台
    if (
      ['youtube.com', 'bilibili.com', 'vimeo.com', 'youku.com'].includes(domain)
    ) {
      return 'video'
    }
