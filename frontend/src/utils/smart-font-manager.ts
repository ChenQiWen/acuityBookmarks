...
class SmartFontManager {
  ...
  processPageElements() {
    // 处理系统UI元素
    const systemElements = document.querySelectorAll(
      '.system-ui, .navigation, .toolbar, .menu, .dialog-header, .button, .form-label, .status-text'
    )
    ...
    const contentElements = document.querySelectorAll(
      '.user-content, .bookmark-item, .bookmark-title, .folder-name, .search-results'
    )
          ...
          mutation.addedNodes.forEach(node => {
            if (node.nodeType === Node.ELEMENT_NODE) {
              ...
              const childElements = element.querySelectorAll(
                '.system-ui, .navigation, .toolbar, .menu, .dialog-header, .button, .form-label, .status-text, .user-content, .bookmark-item, .bookmark-title, .folder-name, .search-results'
              )
              childElements.forEach(child => {
                const childIsSystemUI =
                  child.closest(
                    '.system-ui, .navigation, .toolbar, .menu, .dialog-header, .button, .form-label, .status-text'
                  ) !== null
