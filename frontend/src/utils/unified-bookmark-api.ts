 ...
 *
 * 职责：
 * - 为页面/组件提供一致的数据访问入口（消息通信 + IndexedDB 回退）
 * - 封装健康检查、连接管理与错误重试
 * - 仅做数据编排，不承载“搜索/差异/执行”等业务实现
 *
 * 与应用层/核心层关系：
 * - 搜索/差异/执行等能力已在 Application/Core 层实现；页面应优先使用
 *   应用层服务（如 searchAppService、bookmarkChangeAppService），本 API 仅在
...
export class UnifiedBookmarkAPI {
  ...
  async searchBookmarks(
    ...
    try {
      ...
      logger.info(
        'UnifiedAPI',
        `搜索完成: ${results.length}条结果, 耗时: ${executionTime.toFixed(2)}ms`
      )

      // 自动添加搜索历史（统一API默认为管理页面来源）
      try {
        await this.addSearchHistory(
          query,
          results.length,
          executionTime,
          'management'
   ...
   */
  async generateEmbeddings(force: boolean = false): Promise<{
    success: boolean
    processed?: number
    total?: number
    duration?: number
    ...
    await this._ensureReady()
    const response = await this._sendMessage<
      ApiResponse<{ processed: number; total: number; duration: number }>
    >({
   ...
   */
  async vectorizeSync(
    options: { batchSize?: number; timeout?: number; force?: boolean } = {}
  ): Promise<{
    success: boolean
    upserted?: number
    batches?: number
  ...
  async getEmbeddingCoverage(): Promise<{
    ...
    await this._ensureReady()
    const response = await this._sendMessage<
      ApiResponse<{ total: number; withEmbeddings: number; missing: number }>
    >({
  ...
  async getBookmarkHealthOverview(): Promise<BookmarkHealthOverview> {
    ...
    const response = await this._sendMessage<
      ApiResponse<BookmarkHealthOverview>
    >({
  ...
  async crawlAndCacheBookmark(
    ...
    const resp = await fetch(
      `${API_CONFIG.API_BASE}${API_CONFIG.ENDPOINTS.crawl}`,
      {
        method: 'POST',
        headers: {
    ...
    const httpStatus: number | undefined =
      m.crawlStatus?.httpStatus ?? m.httpStatus
    const statusGroup: CrawlMetadataRecord['statusGroup'] = (() => {
      const s = Number(httpStatus || 0)
      if (s >= 200 && s < 300) return '2xx'
      if (s >= 300 && s < 400) return '3xx'
  ...
  async addSearchHistory(
    ...
    try {
      await indexedDBManager.addSearchHistory(
        query,
        results,
        executionTime,
        source
  ...
  async getBookmarkStats() {
    try {
      ...
      try {
        ...
        const duplicates = Array.from(urlCounts.values()).filter(
          c => c > 1
        ).length
...
export class SidePanelBookmarkAPI extends PageBookmarkAPI {
  ...
  async searchBookmarks(query: string, bookmarkTree?: BookmarkTreeNode[]) {
    ...
    try {
      return await this.api.searchBookmarks(query, {
        limit: 100,
        includeUrl: true,
        includeDomain: true
      })
  ...
  private _memorySearch(
    ...
    const searchInNodes = (
      ...
      for (const node of Array.isArray(nodes) ? nodes : []) {
        ...
        const titleLower: string = (node.titleLower || node.title || '')
          .toString()
          .toLowerCase()
        const urlLower: string = (node.urlLower || node.url || '')
          .toString()
          .toLowerCase()
        const domainLower: string = (node.domain || '').toString().toLowerCase()
        const keywords: string[] = Array.isArray(node.keywords)
          ? node.keywords
          : []
        ...
        if (node.url) {
          ...
          const keywordMatch = keywords.some(
            k => typeof k === 'string' && k.includes(q)
          )
          ...
          if (titleMatch || domainMatch || urlMatch || keywordMatch) {
            ...
            // 路径与 pathIds 优先使用预计算
            const path: string[] = Array.isArray(node.path)
              ? node.path
              : [...currentPath]
          ...
          const newPath = Array.isArray(node.path)
            ? node.path
