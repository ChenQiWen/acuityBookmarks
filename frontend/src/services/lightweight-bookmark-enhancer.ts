...
export class LightweightBookmarkEnhancer {
  ...
  async enhanceBookmarks(
    ...
    let attemptsRemaining = Math.max(
      0,
      CRAWLER_CONFIG.DAILY_LIMIT - daily.count
    )
    if (attemptsRemaining <= 0) {
      logger.info(
        'LightweightEnhancer',
        `📉 已达到每日爬取上限(${CRAWLER_CONFIG.DAILY_LIMIT})，今天不再发起网络爬取`
      )
      return results
    }
    for (let i = 0; i < bookmarks.length; i += batchSize) {
      ...
      const batchPromises = allowedBatch.map(bookmark =>
        this.enhanceBookmark(bookmark)
      )
      ...
      if (i + batchSize < bookmarks.length) {
        await new Promise(resolve =>
          setTimeout(resolve, CRAWLER_CONFIG.BATCH_INTERVAL_MS)
        )
      }
  ...
  private async crawlAndCache(
    ...
    try {
      ...
      if (
        CRAWLER_CONFIG.MODE === 'serverless' ||
        CRAWLER_CONFIG.MODE === 'hybrid'
      ) {
        try {
          ...
          if (crawlResult) {
            logger.info(
              'LightweightEnhancer',
              `✅ Serverless爬取成功: ${bookmark.url} (${Date.now() - startTime}ms)`
            )
          }
        } catch (serverlessError) {
          logger.warn(
            'LightweightEnhancer',
            `⚠️ Serverless爬虫失败，尝试本地爬虫: ${bookmark.url}`,
            serverlessError
          )
        }
      }
      ...
      if (!crawlResult) {
        try {
          ...
          if (crawlResult) {
            logger.info(
              'LightweightEnhancer',
              `✅ 本地爬取成功: ${bookmark.url} (${Date.now() - startTime}ms)`
            )
          }
        } catch (localError) {
          logger.warn(
            'LightweightEnhancer',
            `⚠️ 本地爬虫也失败: ${bookmark.url}`,
            localError
          )
        }
      }
   ...
   * 📊 解析轻量级元数据 (HTML解析)
   */
  private async extractLightweightMetadata(
    html: string,
    url: string
  ): Promise<{
    ...
    // 提取meta description
    const descriptionMeta = doc.querySelector(
    ...
    // 提取OpenGraph数据
    const ogTitleMeta = doc.querySelector(
      'meta[property="og:title"]'
    ) as HTMLMetaElement
    const ogTitle = ogTitleMeta?.content?.trim() || ''

    const ogDescriptionMeta = doc.querySelector(
      'meta[property="og:description"]'
    ) as HTMLMetaElement
    const ogDescription = ogDescriptionMeta?.content?.trim() || ''

    const ogImageMeta = doc.querySelector(
      'meta[property="og:image"]'
    ) as HTMLMetaElement
    const ogImage = ogImageMeta?.content?.trim() || ''

    // 🆕 提取OpenGraph网站名称
    const ogSiteNameMeta = doc.querySelector(
  ...
  // === 保存到缓存 ===
  private async saveToCacheInternal(
    metadata: LightweightBookmarkMetadata
  ): Promise<void> {
    if (!this.db) return
  ...
  private async saveToUnifiedIndexedDB(
    m: LightweightBookmarkMetadata
  ): Promise<void> {
  ...
  private async deferForIdleIfNeeded(): Promise<void> {
    ...
    if (
      ...
      await new Promise(resolve =>
        setTimeout(resolve, CRAWLER_CONFIG.IDLE_DELAY_MS)
      )
    }
  }
  ...
  private async getDailyCounter(): Promise<DailyCounter> {
    try {
      ...
      const existing =
        await indexedDBManager.getSetting<DailyCounter>(DAILY_COUNTER_KEY)
  ...
  private async incrementDailyCounter(n: number = 1): Promise<void> {
    try {
      ...
      const existing =
        await indexedDBManager.getSetting<DailyCounter>(DAILY_COUNTER_KEY)
      const current: DailyCounter =
        !existing || existing.date !== today
          ? { date: today, count: 0 }
          : existing
      const updated = {
        date: today,
        count: Math.min(current.count + n, CRAWLER_CONFIG.DAILY_LIMIT)
      }
